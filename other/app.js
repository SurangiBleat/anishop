...
app.get('/', function (req, res) {
  let cat = new Promise(function (resolve, reject) {
    con.query(
      "select id,name, description, cost, category_id, shop_id, photo, identification, ref_link from (select id,name, description, cost, category_id, shop_id, photo, identification, ref_link, if(if(@curr_category != category_id, @curr_category := category_id, '') != '', @k := 0, @k := @k + 1) as ind from goods, ( select @curr_category := '' ) v ) goods where ind < 3 limit 30",
